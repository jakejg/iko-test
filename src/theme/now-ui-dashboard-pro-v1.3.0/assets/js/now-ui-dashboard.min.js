  ...
  $(document).ready(function () {
    ...
    0 != $('.tagsinput').length && $('.tagsinput').tagsinput(),
      ...
      0 != $('.selectpicker').length &&
        $('.selectpicker').selectpicker({ iconBase: 'now-ui-icons', tickIcon: 'ui-1_check' }),
      ...
      $('.form-file-multiple .inputFileHidden').change(function () {
        for (var e = '', a = 0; a < $(this).get(0).files.length; ++a)
          a < $(this).get(0).files.length - 1
            ? (e += $(this).get(0).files.item(a).name + ',')
            : (e += $(this).get(0).files.item(a).name);
        $(this).siblings('.inputFileVisible').val(e);
      }),
    ...
    (seq = seq2 = 0),
      0 == $('.full-screen-map').length &&
        ...
        $navbar.hasClass('bg-white') && 991 < $(window).width()
          ? 0 == scrollElement.scrollTop() && $navbar.removeClass('bg-white').addClass('navbar-transparent')
  ...
  (nowuiDashboard = {
    ...
    checkScrollForTransparentNavbar: debounce(function () {
      scrollElement.scrollTop() >= scroll_distance
        ? transparent &&
          ((transparent = !1), $('.navbar[color-on-scroll]').removeClass('navbar-transparent').addClass('bg-white'))
        : transparent ||
          ((transparent = !0),
          (0 !== $(".navbar.fixed-top .navbar-toggler[aria-expanded='false']").length || 991 < $(window).width()) &&
    ...
    startAnimationForLineChart: function (e) {
      e.on('draw', function (e) {
        'line' === e.type || 'area' === e.type
          ...
          : 'point' === e.type &&
            (seq++,
            e.element.animate({ opacity: { begin: seq * delays, dur: durations, from: 0, to: 1, easing: 'ease' } }));
      }),
        (seq = 0);
    },
    startAnimationForBarChart: function (e) {
      e.on('draw', function (e) {
        'bar' === e.type &&
          (seq2++,
          e.element.animate({ opacity: { begin: seq2 * delays2, dur: durations2, from: 0, to: 1, easing: 'ease' } }));
      }),
        (seq2 = 0);
    },
    showSidebarMessage: function (e) {
      try {
        $.notify(
          { icon: 'now-ui-icons ui-1_bell-53', message: e },
